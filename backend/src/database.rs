...
pub fn get_user_account_details(
    user_id: i32,
) -> Result<Option<Vec<UserAccountDetails>>, Box<dyn Error>> {
    let mut user_account_details_list: Vec<UserAccountDetails> = Vec::new();

    let connection = get_database_connection()?;
    let mut statement =
        connection.prepare("SELECT * FROM users_account_details WHERE user_id = ?1")?;
    ...
    match user_account_details_list.len() > 0 {
        true => Ok(Some(user_account_details_list)),
        false => Ok(None),
    }
}
...
pub fn get_bank_balance() -> Result<Option<Vec<BankBalance>>, Box<dyn Error>> {
    ...
    match balance_list.len() > 0 {
        true => Ok(Some(balance_list)),
        false => Ok(None),
    }
}

pub fn get_user_spending_history(
    user_id: i32,
) -> Result<Option<Vec<UserSpendHistory>>, Box<dyn Error>> {
    let mut user_spending_history_list: Vec<UserSpendHistory> = Vec::new();

    let connection = get_database_connection()?;
    let mut statement =
        connection.prepare("SELECT * FROM users_spend_history WHERE user_id = ?1")?;
    let query_result = statement.query_map([&user_id], |row| {
        Ok(UserSpendHistory {
            user_id: row.get(1)?,
    ...
    match user_spending_history_list.len() > 0 {
        true => Ok(Some(user_spending_history_list)),
        false => Ok(None),
    }
}
...
pub fn insert_user(user: Users) -> Result<String, Box<dyn Error>> {
    ...
    let mut statement = connection.prepare(
        "INSERT INTO users (full_name, username, email, password, api_key) VALUES (?, ?, ?,?,?)",
    )?;
...
pub fn insert_bank_balance(bank_balance: BankBalance) -> Result<(), Box<dyn Error>> {
    ...
    //print!("{}","here");
    let mut statement =
        connection.prepare("INSERT INTO bank_balance (hot_balance, cold_balance) VALUES (?,?)")?;
...
pub fn insert_user_spend(user_spending: UserSpendHistory) -> Result<(), Box<dyn Error>> {
    ...
    let _ = statement.execute((
        ...
        user_spending.source_address,
        user_spending.destination_address,
        user_spending.amount_spent,
        user_spending.fees_amount,
    ))?;

    Ok(())
}

pub fn insert_user_account_details(
    ...
    let mut statement = connection.prepare(
        "INSERT INTO users_account_details (user_id, balance, account_address)
    VALUES (?, ?, ?)",
    )?;
    let _ = statement.execute((
        user_account_details.user_id,
        user_account_details.balance,
        user_account_details.account_address,
    ))?;

    Ok(())
}

pub fn update_user_account_details(
    user_account_details: UserAccountDetails,
    ...
    let mut statement = connection.prepare(
        "UPDATE users_account_details SET balance = ?, account_address = ? WHERE user_id = ?",
    )?;
    let result_count = statement.execute((
        user_account_details.user_id,
        user_account_details.balance,
        user_account_details.account_address,
    ))?;
...
pub fn get_wallet_balance(rpc: &RpcOps) -> Result<Option<f64>, Box<dyn Error>> {
    let my_wallet = rpc.get_balance().unwrap().to_btc();

    Ok(Some(my_wallet))
}

pub fn spend_from_wallet(rpc: &RpcOps, spend: Spend) -> BtcResult<String> {
